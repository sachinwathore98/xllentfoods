  ...
  try {
    ...
    const result = await pool.query(`
      SELECT 
        ...
        END as status
      FROM products p
      LEFT JOIN partner_inventories base_inv ON base_inv.product_id = p.id AND base_inv.user_id = $1
      LEFT JOIN (
...
// --- ROBUST AUTOMATED INVENTORY MOVEMENT ON ORDER COMPLETION ---
async function applyInventoryMovement(orderId, newStatus) {
  try {
    const orderRes = await pool.query("SELECT * FROM orders WHERE id = $1", [orderId]);
    if (orderRes.rows.length === 0) return;
    ...
    // Only process inventory when status is explicitly Completed or Approved
    if (newStatus !== 'Approved' && newStatus !== 'Completed') return;
    ...
    for (let item of items) {
      // 1. If buyer is Super Stockist buying from Admin/Super Admin -> Increase Super Stockist stock
      if (buyer.role === 'super_stockist' && (!seller || seller.role === 'admin' || seller.role === 'superadmin')) {
        await pool.query(`
          INSERT INTO partner_inventories (user_id, product_id, quantity, status)
          VALUES ($1, $2, $3, 'In Stock')
      ...
      // 2. If seller is Super Stockist selling to Distributor/Shop -> Decrease Super Stockist stock
      if (seller && seller.role === 'super_stockist') {
        await pool.query(`
          INSERT INTO partner_inventories (user_id, product_id, quantity, status)
          VALUES ($1, $2, GREATEST(0, -$3), 'In Stock')
...
// Hook into order status update endpoint
app.put('/api/orders/:id/status', async (req, res) => {
  try {
    const { id } = req.params;
    const { status } = req.body;
    ...
    // Trigger live inventory adjustment
    await applyInventoryMovement(id, status);

    res.json({ success: true, message: 'Order status updated and live inventory synchronized' });
  } catch (err) {
...
// Hook into general order update endpoint as well
app.put('/api/orders/:id', async (req, res) => {
  const { id } = req.params;
  const { items, totalAmount, status } = req.body;
  try {
    ...
    res.json({ success: true, message: 'Order successfully updated and inventory synchronized' });
  } catch (err) {
    console.error('Update Order Error:', err);
    res.status(500).json({ error: `Failed to update order in database: ${err.message}` });
  }
