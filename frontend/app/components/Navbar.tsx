  ...
  return (
    <nav className="bg-white/90 backdrop-blur-md border-b border-slate-200 sticky top-0 z-50 shadow-sm">
      <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
        {/* Logo / Brand (Enlarged size) */}
        <a href="/" className="flex items-center gap-3 group">
          {!imgError ? (
            <img 
              src="/images/logo.png" 
              alt="Xllent Foods Logo" 
              className="h-14 sm:h-16 w-auto object-contain group-hover:scale-105 transition duration-300"
              onError={() => setImgError(true)}
            />
          ) : null}
          {/* Fallback Text Logo if image fails to load */}
          <span className={`${!imgError ? 'hidden sm:inline-block' : 'inline-block'} text-xl sm:text-2xl font-black text-slate-900 tracking-tight`}>
            XLLENT FOODS
          </span>
        </a>
